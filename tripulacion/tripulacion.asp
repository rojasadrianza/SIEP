...
<%	
	...
	'Se tom� la decisi�n de utilizar un script para tripulaci�n y otro para pasajero'
...
'RESPONSE.WRITE "codigo-------" & ed_in_bor 	
	...
	'Usando Session ("rand")'
	'Es la mejor soluci�n hasta el momento para evitar que se inserte'
	'al hacer un refresh'
	...
	if guard = "1" then
		...
		if refresco = false then
			...
			End if		
				...
				'//////////////////////////////////////'			
				'Solo nombre y tipo de tripulaci�n no pueden ser null'
				'No se admiten pasaporte, c�dula, visa y/o correo repetidos '
				...
				if cedular <> "" And (cedular <> cedula2r) then
					if Tabla_Vacia("Siep_trip", " Num_cedu_tri = '" & cedular & "' And Num_clie = " & Session("Num_clie")) = false then
						call alerta("La c�dula " & cedular & " se encuentra registrada ")
						err_nums =  err_nums + 1
					End if
				End if
    ...
    '
	'	
	'		hid_tab = hid_tab & hidden("selct_fila" & CStr(ind) & "_22", Replace(rstf("Nom_pais"), "Ñ", "N")) & hidden("selct_fila" & CStr(ind) & "_23", rstf("Fec_nac_tri"))		
	'		
	'		
	'		
	...
	'		end if
	'		
	'		hid_tab = hid_tab & hidden("selct_fila" & CStr(ind) & "_25", Replace(Pais_res_trip, "Ñ", "N")) & hidden("selct_fila" & CStr(ind) & "_26", Pais_res_trip)					
	...
	'				Estado_trip=""  
	'			end if
	'		 hid_tab = hid_tab & hidden("selct_fila" & CStr(ind) & "_29", Replace(Estado_trip, "Ñ", "N")) & hidden("selct_fila" & CStr(ind) & "_30", Estado_trip)						
	...
	'			   ciudad_trip=""  
	'		    end if
	'		 hid_tab = hid_tab & hidden("selct_fila" & CStr(ind) & "_31", Replace(ciudad_trip, "Ñ", "N")) & hidden("selct_fila" & CStr(ind) & "_32", ciudad_trip)			
    ...
    if  not rstf.EOF then
    	...
    	Do Until rstf.EOF	
			...
			hid_tab = rstf("Sex_trip")& "|" & Replace(rstf("Nom_pais"), "Ñ", "N") & "|" & rstf("Fec_nac_tri") & "|" & Replace(Pais_res_trip, "Ñ", "N")
			hid_tab = hid_tab & "|" & Replace(Estado_trip, "Ñ", "N")  & "|" & Replace(ciudad_trip, "Ñ", "N")
